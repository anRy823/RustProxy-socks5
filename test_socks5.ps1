# Simple SOCKS5 test using PowerShell
param(
    [string]$ProxyHost = "127.0.0.1",
    [int]$ProxyPort = 1080
)

Write-Host "Testing SOCKS5 proxy at ${ProxyHost}:${ProxyPort}"
...
try {
    ...
    # Send SOCKS5 greeting (version 5, 2 methods: no auth and username/password)
    Write-Host "Sending SOCKS5 greeting..."
    ...
    elseif ($method -eq 0x00) {
        Write-Host "✅ No authentication required"
    }
    else {
        Write-Host "❌ Unsupported authentication method: $method"
        exit 1
    }
    ...
    Write-Host "✅ CONNECT successful!"
    
    # Try to send a simple HTTP request
    Write-Host "Sending HTTP request..."
    $httpRequest = [System.Text.Encoding]::ASCII.GetBytes("GET /ip HTTP/1.1`r`nHost: httpbin.org`r`nConnection: close`r`n`r`n")
    ...
    if ($stream.DataAvailable) {
        ...
        $bytesRead = $stream.Read($responseBuffer, 0, 1024)
        $httpResponse = [System.Text.Encoding]::ASCII.GetString($responseBuffer, 0, $bytesRead)
        Write-Host "✅ Received HTTP response ($bytesRead bytes):"
        Write-Host $httpResponse.Substring(0, [Math]::Min(200, $httpResponse.Length))
        Write-Host "✅ SOCKS5 proxy test PASSED"
    }
    else {
        Write-Host "⚠️  No HTTP response received (but SOCKS5 protocol worked)"
        Write-Host "✅ SOCKS5 proxy test PARTIALLY PASSED"
    }
}
catch {
